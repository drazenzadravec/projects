 ...
 */
export interface IResponseHandler extends IHandlerBase {
...
export class ResponseHandler implements IResponseHandler {
	...
	/**
     * Response handler api implementation.
     * 
     ...
     * @param {Object}				responseHandlerOptions  A collection of options.
     *        
     * @example
     *  options = {
	 *		debug: true
     ...
     *  }
     */
	constructor(public lakeNiceClient: ILakeNiceClient, public lakeNiceAuthClient: ILakeNiceAuthClient, public responseHandlerOptions: any) {

		// local.
		let self = this;
		let parent = responseHandlerOptions.parent;
		let item;

		let options = responseHandlerOptions || {};
		let config = this.config = {
			debug: false
		};

		// Call WildEmitter constructor.
		wildemitter.mixin(ResponseHandler);
		...
		let logger = function () {
			...
			// possible.
			if (responseHandlerOptions.debug) {
				return responseHandlerOptions.logger || console;
			}
			else {
				// Use your logger which should have its own logic
				// for output. Return the no-op.
				return responseHandlerOptions.logger || mockconsole;
			}
		}();
		...
		if (this.config.debug) {
			...
			this.on('*', function (event, val1, val2, val3) {
				...
				// Log the event.
				logger.log('Response Handler Event:', event, val1, val2);
			});
		}
