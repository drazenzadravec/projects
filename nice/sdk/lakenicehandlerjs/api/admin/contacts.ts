...
/**
* Contacts handler interface.
*/
export interface IHContacts {
...
/**
 * Contacts handler implementation.
 */
export class HContacts implements IHContacts {
     ...
     * @param {Object}				contactsHandlerOptions		A collection of options.
     *        
     * @example                          
     *  options = {
	 *		debug: true
     *  }
     */
	constructor(public lakeNiceClient: ILakeNiceClient, public contactsHandlerOptions: any) {

		// local.
		let self = this;
		let parent = contactsHandlerOptions.parent;
		let item;

		let options = contactsHandlerOptions || {};
	...
	assignRequestOptions(uniqueID: string, requestData?: any): any {
		...
		if (requestData === null) {
			...
			switch (requestMethod) {
				...
				case "startMonitoringPhoneCallAsync": {
					options.data = requestData;
					return options;
				}
				case "assignTagsAsync": {
					options.data = requestData;
					return options;
				}
				case "createSignalAsync": {
					options.data = requestData;
					return options;
				}
				case "getSMSTranscriptAsync": {
					options.params = requestData;
					return options;
				}
				case "getPastSMSContactsAsync": {
					options.params = requestData;
					return options;
				}
				default: {
