...
var captureSelect = null;
var selectors = null;
var captureRefreshButton = null;
...
function init() {
    ...
    if (conferenceContactList) {
        conferenceContactList.push("kathy");
        conferenceContactList.push("drazen");
        conferenceContactList.push("sindy");
	...
	captureRefreshButton = document.getElementById("captureSourceRefresh");
	...
	captureSelect = document.getElementById("captureSource");
	...
	// Refresh capture source.
	captureRefreshButton.addEventListener("click", refreshCaptureSource);
 ...
 * Add capture sources.
 */
function addCaptureSource(source) {
 ...
 */
function refreshCaptureSource() {
  
	var length = captureSelect.options.length;
	for (i = length - 1; i >= 0; i--) {
		captureSelect.remove(i);
	}
    ...
    if (useScreenElement.checked || useWindowElement.checked) {
        ...
        if (useScreenElement.checked) {
			...
			var constraints = {
				...
				video: {
					mandatory: {
						chromeMediaSourceId: captureSelect.value,
